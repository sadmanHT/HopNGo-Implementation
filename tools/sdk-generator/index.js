...
async function generateTypes(service) {
  try {
    console.log(chalk.blue(`🔧 Generating types for ${service.name}...`));
    ...
    console.log(chalk.green(`✅ Generated types: ${typesPath}`));
    return { service: service.name, success: true, typesPath };
    
  } catch (error) {
    console.log(chalk.red(`❌ Failed to generate types for ${service.name}: ${error.message}`));
...
async function generateClient(service) {
  try {
    console.log(chalk.blue(`🔧 Generating client for ${service.name}...`));
...
`;
    ...
    console.log(chalk.green(`✅ Generated client: ${clientPath}`));
    
    return { service: service.name, success: true, clientPath };
    
  } catch (error) {
    console.log(chalk.red(`❌ Failed to generate client for ${service.name}: ${error.message}`));
    return { service: service.name, success: false, error: error.message };
  }
}
...
`;
    ...
    console.log(chalk.green(`📝 Generated SDK index: ${indexPath}`));
    
  } catch (error) {
    console.log(chalk.red(`❌ Failed to generate index file: ${error.message}`));
  }
}

async function main() {
  console.log(chalk.cyan('🚀 HopNGo SDK Generator'));
  ...
  console.log(chalk.cyan(`\n🎉 Generated SDKs for ${successCount}/${services.length} services!`));
...
// Run the generator
main().catch(error => {
  console.error(chalk.red('❌ Failed to generate SDKs:'), error);
  process.exit(1);
});