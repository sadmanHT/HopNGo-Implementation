...
/**
 * Mock implementation of BkashPaymentProvider for testing purposes.
 * This mock simulates BKash API responses without making actual API calls.
 */
@TestComponent
@Profile("test")
public class MockBkashPaymentProvider implements PaymentProvider {
    ...
    public PaymentIntentResponse createPaymentIntent(Order order) {
        // Simulate BKash payment intent creation
        String paymentId = "bkash_pi_" + UUID.randomUUID().toString().substring(0, 8);
        String clientSecret = "bkash_secret_" + UUID.randomUUID().toString().substring(0, 16);
    ...
    public boolean verifyWebhook(HttpServletRequest request) {
        ...
        String signature = request.getHeader("X-Bkash-Signature");
